...
vi.mock('../../src/components/GifGrid', () => ({
  GifGrid: vi.fn(({ category }) => (
    <div data-testid='gif-grid'>{`GifGrid for ${category}`}</div>
  )),
}));
...
describe('GifExpertApp component', () => {
  ...
  it('should add a new category and render a GifGrid for it', async () => {
    ...
    // Wait for the GifGrid for SAITAMA to appear
    expect(screen.getByText('GifGrid for SAITAMA')).toBeTruthy();

    expect(GifGrid).toHaveBeenCalledWith(
      expect.objectContaining({ category: 'SAITAMA' }),
      {}
    );
  });
  ...
  it('should not add the same category twice', async () => {
    ...
    const gokuCalls = gifGridCalls.filter(
      (call) => call[0].category === 'GOKU'
    );
  ...
  it('should add multiple different categories and render GifGrids for them', async () => {
    ...
    expect(GifGrid).toHaveBeenCalledWith(
      expect.objectContaining({ category: 'NARUTO' }),
      {}
    );
    ...
    expect(GifGrid).toHaveBeenCalledWith(
      expect.objectContaining({ category: 'BLEACH' }),
      {}
    );
  });
});
